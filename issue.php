<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
    {
      ...
      {
      $update = "UPDATE `books` SET `issuedto` = '$username' WHERE `books`.`sno` = '$id' " ;
      $execute = mysqli_query($conn,$update);
      $issued = true;
      $showAlert = "Book Isseued Successfully";
      }
      else
      {
        $not_issued = true;
...
<div class="container mt-4 col-lg-4 bg-light">
       ...
       <main class="form-signin text-center">
         <form method="post" action="">
           <h1 class="h3 mb-3 fw-bold">Issue Books</h1>
           ...
           <div class="form-floating mt-3">
             <input type="text" class="form-control" id="username" name="username" placeholder="Book ID" required></input>
             <label for="username">Username</label>
           </div>
           <input type="hidden" name="id" value = " <?php echo $_GET['id']; ?>" ></input>
          
                
           <button class="w-100 btn btn-lg btn-primary mt-2" type="submit">Issue</button>
